layui.define(["jquery", "layer"], function(exports) {
	...
	var obj = function(config) {
		...
		this.addLi = function(arr, ele, index) {
			if(arr==undefined){
				return;
			}
			var ul_ele = ele.find(".layui-fy-city-pro:eq(" + index + ") ul");
			//清除
			for(var i = index; i <= 2; i++) {
				ele.find(".layui-fy-city-pro:eq(" + i + ") ul li:gt(0)").remove();
				ele.find(".layui-fy-city-pro:eq(" + i + ") ul li:eq(0)").addClass("highlight");
				ele.find(".layui-fy-city-pro:eq(" + i + ") ul").css("transform", "perspective(1000px) rotateY(0deg) rotateX(0deg)");
			}
			for(var i = 0; i < arr.length; i++) {
				...
				var temli = '<li class="' + vcss + '" style="transform-origin: center center -90px; transform: translateZ(90px) rotateX(' + ((i + 1) * -20) + 'deg);" data-val="' + arr[i] + '" >' + arr[i] + '</li>';
				ul_ele.append(temli);
			}
		}
		this.addLiOne = function(arr, ele) {
		...
		this.setVal = function(data) {
			var o = this,
				c = o.config;
			$E = $(c.elem), $T = $(c.trigger);

			if(data != "") {
				var def_arr = data.split(',');
				if(def_arr.length == 3) {
					//省
					var pro_index = $.inArray(def_arr[0], dsy.Items["0"]);
					var ul_child = $E.find("ul:eq(0)");
					var cindex = pro_index + 1;
					o.setPoint(ul_child, cindex);
					var cityArr = dsy.Items["0_" + pro_index];
					o.addLi(cityArr, $E, 1);
					//市
					ul_child = $E.find("ul:eq(1)");
					cindex = $.inArray(def_arr[1], dsy.Items["0_" + pro_index]);
					o.setPoint(ul_child, cindex+1); 
					var areaArr = dsy.Items["0_" + pro_index + "_" + cindex];
					o.addLi(areaArr, $E, 2);
					//区域
					ul_child = $E.find("ul:eq(2)");
					cindex = $.inArray(def_arr[2], dsy.Items["0_" + pro_index+"_"+cindex]);
					o.setPoint(ul_child, cindex+1); 
				}
			}
		}
		this.setPoint=function(ul_child, cindex) {
			ul_child.find("li").removeClass("highlight").eq(cindex).addClass("highlight");
			ul_child.css("transform", "perspective(1000px) rotateY(0deg) rotateX(" + (cindex * 20) + "deg)");
			//当前选择的前4个和后4个显示
			ul_child.find("li").removeClass("visible");
			if(cindex >= 4) {
				for(var i = cindex - 4; i < cindex + 4; i++) {
					if(i != cindex) {
						ul_child.find("li").eq(i).addClass("visible");
					}
				}
			} else {
				for(var i = 0; i < cindex + 4; i++) {
					if(i != cindex) {
						ul_child.find("li").eq(i).addClass("visible");
					}
				}
		...
		obj.prototype.render = function() {
			...
			//添加专属的style
			if($('#layui-fy-city-style').length == 0) {
				var style = '.layui-layer-shade{top:0;left:0;width:100%;height:100%;_height:expression(document.body.offsetHeight+"px")}.layui-layer,.layui-layer-shade{position:fixed;_position:absolute;pointer-events:auto}.layui-fy-city{z-index:1000;position:fixed;z-index:999999;bottom:0;transform:translateY(300px);border-top:1px solid #ccc;left:0;background-color:white;box-shadow:rgba(0,0,0,0.1) 0 -5px 7px 0;width:100%;transition:.3s}.layui-fy-city.active{transform:translateY(0px)}.layui-fy-city-header{font-size:16px;font-weight:600;padding:6px;color:#333;border-bottom:1px solid #e2e2e2}.layui-fy-city-body{position:relative;width:100%;height:200px}.layui-fy-city .layui-fy-city-pro{user-select:none;position:relative;width:33.3%;height:100%;float:left;margin:0;border-width:initial;border-style:none;border-color:initial;border-image:initial}.layui-fy-city-picker-inner{position:relative;width:100%;height:100%;-webkit-mask-box-image:-webkit-linear-gradient(bottom,transparent,transparent 5%,#fff 20%,#fff 80%,transparent 95%,transparent);box-sizing:border-box;overflow:hidden}.layui-fy-city-pciker-list,.layui-fy-city-pciker-rule{box-sizing:border-box;width:100%;height:36px;line-height:36px;position:absolute;left:0;top:50%;padding:0;margin:-18px 0 0}.layui-fy-city-pciker-list{z-index:1;transform-style:preserve-3d;transform:perspective(750pt) rotateY(0deg) rotateX(0deg)}.layui-fy-city-pciker-rule-ft{z-index:2;border-top:1px solid rgba(58,168,90,0.58);border-bottom:1px solid rgba(58,168,90,0.58)}.layui-fy-city-pciker-list li{width:100%;height:100%;position:absolute;text-align:center;vertical-align:middle;backface-visibility:hidden;font-size:1pc;font-family:"Helvetica Neue",Helvetica,Arial,sans-serif;color:#888;white-space:nowrap;text-overflow:ellipsis;cursor:default;visibility:hidden;padding:0 8px}.layui-fy-city-pciker-list li.highlight,.layui-fy-city-pciker-list li.visible{visibility:visible}';
				$('<style id="layui-fy-city-style"></style>').text(style).appendTo($('head'));
			};

			var city = $('<div class="layui-fy-city ">');
			var c_head = $('<div class="layui-fy-city-header"> 省市区选择</div>');
			var c_foot = $('<div class="layui-fy-city-foot"><a class="layui-btn layui-btn-fluid">确定</a></div>');
			var c_body = $('<div class="layui-fy-city-body"></div>');
			var opt0 = ["省份", "地级市", "市、县级市"]; //初始值
			for(var i = 0; i < opt0.length; i++) {
				var temOne = '<div class="layui-fy-city-pro">';
				temOne += '<div class="layui-fy-city-picker-inner">';
				temOne += '<div class="layui-fy-city-pciker-rule layui-fy-city-pciker-rule-ft"></div>';
				temOne += '	<ul class="layui-fy-city-pciker-list" style="transform: perspective(1000px) rotateY(0deg) rotateX(0deg);">';
				temOne += '<li class="highlight visible" style="transform-origin: center center -90px; transform: translateZ(90px) rotateX(0deg);" data-val="">' + opt0[i] + '</li>';
				temOne += '</ul></div></div>';
				c_body.append(temOne);
			}
			city.append(c_head).append(c_body).append(c_foot);

			//确认信息
			c_foot.click(function() {

				var area = $E.find(".highlight");
				var txt = "";

				for(var i = 0; i < area.length; i++) {
					var val = $(area[i]).attr("data-val");
					if(val == "") {
						layer.msg("信息不完整！");
						return;
					}
			...
			var ele_pro = c_body.find("ul:eq(0)");
			//添加li
			o.addLiOne(province, ele_pro);

			$E.append(city);
			$E.append('<div class="layui-layer-shade" style="z-index: 999; background-color: rgb(0, 0, 0); opacity: 0.8;display: block;"></div>');
			$E.hide();
			//初始值
			o.setVal(c.data);
			...
			$T.focus(function() {
				$E.show(500);
				$E.find(".layui-fy-city").addClass("active");
			});
			$E.on("click", ".layui-layer-shade", function() {
				$(this).parent().find(".layui-fy-city").removeClass("active");
				$(this).parent().hide(500);
			});
			//
			var list = $(".layui-fy-city-pro");
			//ty 触发时位置
			var ty = 0;
			for(var i = 0; i < list.length; i++) {
				...
				list[i].addEventListener("touchstart", function(e) {
					...
					ty = touch.clientY;
					var flg = $(touch.target).hasClass("layui-fy-city-pro");
					var p;
					if(flg) {
						p = $(touch.target);
					} else {
						p = $(touch.target).parents(".layui-fy-city-pro");
					}
					var ul_child = p.find(".layui-fy-city-pciker-list");
					var sindex = p.find(".highlight").index();
					var index_count = p.find("li").length;
					var sel_index = sindex * 20;

					//定位
					var rot = 0;
					p[0].addEventListener("touchmove", function(e) {
						e.preventDefault();
						var touch = e.touches[0];
						var diff = touch.clientY - ty;
						var showNext = sindex + (diff % 20);
						//var findex=sindex-4-
						//							ul_child.find("li").removeClass("visible");
						//							for(var i=showNext-4;i=showNext+4;i++){
						//								ul_child.find("li").eq(i).addClass("visible");
						//							}
						//console.log(diff);
						rot = sel_index - diff;

						$(ul_child).css("transform", "perspective(1000px) rotateY(0deg) rotateX(" + rot + "deg)");
						//console.log(rot);
					}, false);

					p[0].addEventListener("touchend", function(e) {
						e.preventDefault();
						var touch = e.touches[0];
						...
						if(rot >= index_count * 20) {
							rot = index_count * 20 - 20;
						}
						...
						//移动了几个
						var mindex = (rot - sel_index) / 20;
						var cindex = sindex + mindex;
						ul_child.find("li").removeClass("highlight").eq(cindex).addClass("highlight");
						ul_child.css("transform", "perspective(1000px) rotateY(0deg) rotateX(" + rot + "deg)");
						//当前选择的前4个和后4个显示
						ul_child.find("li").removeClass("visible");
						...
						//级别
						var area_txt = ul_child.find(".highlight").text();
						if(p.index() == 0) {
							var pro_index = $.inArray(area_txt, dsy.Items["0"]);
							var cityArr = dsy.Items["0_" + pro_index];
							o.addLi(cityArr, $E, p.index() + 1);
						}
						if(p.index() == 1) {
							var pro_txt = $E.find("ul:eq(0) .highlight").text();
							var pro_index = $.inArray(pro_txt, dsy.Items["0"]);
							var city_index = $.inArray(area_txt, dsy.Items["0_" + pro_index]);
							var areaArr = dsy.Items["0_" + pro_index + "_" + city_index];
							o.addLi(areaArr, $E, p.index() + 1);
						}
